...
const screenResult = document.getElementById("screen-content");
...
const writeResult = () =>{
    ...
    if(screenResult.textContent.length > 8){
        screenResult.style.fontSize = "2rem";
...
const changeSign = () =>{
    ...
    if(!isNaN(lastValue())){
        for (let i = screenOperation.textContent.length - 1; i > 0; i--){
            if(isNaN(screenOperation.textContent[i] || screenOperation.textContent[i] == ")") ){
                position = i + 1;
                break;
            }
        }
...
buttons.addEventListener("click", e =>{
    if(e.target.textContent !== "" && lastValue() != ")"){
